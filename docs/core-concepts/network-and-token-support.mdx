...
Dollar-string pricing (`"$0.01"`) is not yet supported for TVM networks. Use an explicit `AssetAmount` with the jetton minter address instead.
...
* **Any EVM chain** is supported via `eip155:<chainId>` — plus Solana, TON, Stellar, Aptos, and Hedera
