...
x402 supports three asset transfer methods on EVM, selected based on token capabilities and account type:

| Method | When Used | How It Works | Usage Semantics |
|--------|-----------|--------------|-----------------|
| **EIP-3009** | Tokens with `transferWithAuthorization` (e.g., USDC) | Single off-chain signature, simplest flow | One-time use |
| **Permit2** | Any ERC-20 token | Uses Uniswap's [Permit2](https://docs.uniswap.org/contracts/v4/deployments) contract + `x402ExactPermit2Proxy` | One-time use |
| **ERC-7710** | Smart accounts with delegation support (e.g., ERC-4337, ERC-7579) | Uses ERC-7710 smart contract delegation | One-time use and multi-use |

All methods provide gasless transfers (facilitator sponsors gas) and signature-based authorization. EIP-3009 is preferred when available (simpler, no approval step). Permit2 serves as the universal fallback for any ERC-20 token. ERC-7710 is the option for smart accounts with delegation capabilities, and uniquely supports reusable (multi-use) payment authorizations.
...
* Any ERC-20 token works on any EVM facilitator (via EIP-3009 or Permit2); smart accounts can also use ERC-7710
