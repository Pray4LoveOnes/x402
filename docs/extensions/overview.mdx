...
|-----------|------|-------------|-------------|
| [Auth Hints](./auth-hints) | Server + Client | Provides authentication metadata in 402 responses so clients can complete auth before submitting payment, eliminating unnecessary round trips. | Spec only |
